...
function addToNginx(serviceName,port){
    shell.rm('-v', `/etc/nginx/sites-available/${serviceName}`);
    shell.rm('-v', `/etc/nginx/sites-enabled/${serviceName}`);


    shell.cp('-v', './__template__', `/etc/nginx/sites-available/${serviceName}`);


// Replace DOMAIN name in nginx template

    shell.sed('-i',`s/__DOMAIN__/${serviceName}/g`,  `/etc/nginx/sites-available/${serviceName}`);
    shell.sed('-i', `s/__PORT__/${port}/g`, `/etc/nginx/sites-available/${serviceName}`);


    shell.ln('-s',`/etc/nginx/sites-available/${serviceName}`, `/etc/nginx/sites-enabled/${serviceName}`);
